      ...
      <div className="rounded-lg overflow-hidden shadow-card hover:shadow-elevated transition-all duration-300 bg-card hover:scale-102 transform cursor-pointer">
        ...
        <div className="p-3 sm:p-4 space-y-2.5 sm:space-y-3">
          <div className="flex justify-between items-start gap-2">
            <h3 className="font-playfair text-base sm:text-lg font-bold text-foreground line-clamp-1">
              {name}
            </h3>
          ...
          <div className="flex flex-wrap gap-1">
            {amenities.slice(0, 2).map((amenity, idx) => (
              <span
                key={idx}
                className="text-xs bg-muted text-muted-foreground px-1.5 sm:px-2 py-0.5 sm:py-1 rounded"
              >
                {amenity}
              </span>
            ))}
